pipeline{
	...
	stage('Archive'){
     ...
     stage('Deploy WAR'){
      ...
      stage('Deploy Docker'){
          steps{
              sh "docker run -p 7474:8080 -v ${env.JENKINS_HOME}/jobs/${env.JOB_NAME}/builds/${env.BUILD_NUMBER}/archive/spring-boot-package-war/target:/usr/local/tomcat/webapps -d tomcat "
          }
      } 
     }
